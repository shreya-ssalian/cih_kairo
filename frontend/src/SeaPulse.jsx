...
import { useBackendSimulation } from './api/useBackendSimulation';
...
function SettingsPage({ trailsOn, setTrailsOn, soundOn, setSoundOn, tickRate, setTickRate, backendConnected }) {
  return (
    <div className="sp-cols" style={{ alignItems: 'flex-start' }}>
      ...
      <Panel title="Architecture" icon={<Navigation size={16} />} style={{ flex: 1, minWidth: 280 }}>
        <div className="sp-toggle-row" style={{ borderBottom: 'none', paddingTop: 0 }}>
          <span>Backend connection</span>
          <Badge text={backendConnected ? 'Connected' : 'Offline (local fallback)'} color={backendConnected ? 'var(--turquoise)' : 'var(--coral)'} />
        </div>
        {backendConnected ? (
          <p className="sp-copy">
            This session is fed live by the FastAPI backend over a WebSocket — ship movement, geofence
            (point-in-polygon) detection, suspicion/ecosystem scoring, and the alert engine all run
            server-side, in one shared simulation every connected client sees, instead of purely inside
            this browser tab.
          </p>
        ) : (
          <p className="sp-copy">
            The backend isn't reachable right now, so this tab has fallen back to running the simulation
            entirely client-side (an in-memory tick loop), the same as the original standalone build.
            Start the FastAPI server and this page will pick it up automatically.
          </p>
        )}
        <p className="sp-copy">Frontend stack: React, Recharts, Lucide icons, Leaflet, custom SVG ocean-chart rendering.</p>
      </Panel>
    </div>
  );
}
...
export default function SeaPulseApp() {
  ...
  // Real backend integration: if a FastAPI server is reachable at VITE_API_URL
  // (default http://localhost:8000) this hook connects over WebSocket and
  // becomes the source of truth for the whole simulation. If it's not
  // reachable, `backend.connected` stays false and every page below keeps
  // reading from the original in-browser simulator further down this file,
  // so the app still works standalone exactly like before.
  const backend = useBackendSimulation();
  const useBackend = backend.connected;

  const effectiveZones = useBackend ? backend.zones : zones;
  const effectiveShips = useBackend ? backend.ships : ships;
  const effectiveAlerts = useBackend ? backend.alerts : alerts;
  const effectiveTick = useBackend ? backend.tick : tick;
  const effectiveRunning = useBackend ? backend.running : running;
  const effectiveShipTarget = useBackend ? backend.shipTarget : shipTarget;
  const effectiveTickRate = useBackend ? backend.tickRateMs : tickRate;
  const effectiveLiveStats = useBackend ? backend.liveStats : liveStats;
  const effectiveStatsHistory = useBackend ? backend.statsHistory : statsHistory;
  ...
  // When the backend is driving the simulation, sound alerts off of its
  // alert feed the same way the local loop does off of `newAlertsAcc`.
  const prevBackendAlertIdRef = useRef(null);
  useEffect(() => {
    if (!useBackend || !soundOn) return;
    const latest = backend.alerts[0];
    if (!latest || latest.id === prevBackendAlertIdRef.current) return;
    prevBackendAlertIdRef.current = latest.id;
    if (latest.severity === 'critical' || latest.severity === 'high') beep();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [useBackend, soundOn, backend.alerts]);
  ...
  useEffect(() => {
    if (useBackend) return; // backend is driving ticks; don't also run the local loop
    const id = setInterval(doTick, tickRate);
    return () => clearInterval(id);
  }, [doTick, tickRate, useBackend]);
  ...
  const localRunBenchmark = (numMessages = 300000) => {
  ...
  // ---- Unified action wrappers -------------------------------------------
  // Every handler below routes to the real backend (REST calls; the
  // resulting state change arrives back over the WebSocket) when connected,
  // and otherwise falls back to the original in-browser simulation logic
  // defined above. Child pages (GeofencePage, SimulatorPage, SettingsPage)
  // don't need to know which mode is active — they just call these.

  const handleSetZones = useCallback((updater) => {
    if (useBackend) {
      const current = backend.zones;
      const next = typeof updater === 'function' ? updater(current) : updater;
      const currentIds = new Set(current.map(z => z.id));
      const nextIds = new Set(next.map(z => z.id));
      next.filter(z => !currentIds.has(z.id)).forEach(z => {
        backend.addZone({ name: z.name, type: z.type, points: z.points }).catch(err => console.error('addZone failed', err));
      });
      current.filter(z => !nextIds.has(z.id)).forEach(z => {
        backend.deleteZone(z.id).catch(err => console.error('deleteZone failed', err));
      });
    } else {
      setZones(updater);
    }
  ...
  const handleSetRunning = useCallback((updater) => {
    if (useBackend) {
      const next = typeof updater === 'function' ? updater(backend.running) : updater;
      if (next) backend.start().catch(err => console.error('start failed', err));
      else backend.pause().catch(err => console.error('pause failed', err));
    } else {
      setRunning(updater);
    }
  }, [useBackend, backend]);

  const handleSetShipTarget = useCallback((value) => {
    if (useBackend) backend.setShipTarget(value).catch(err => console.error('setShipTarget failed', err));
    else setShipTarget(value);
  }, [useBackend, backend]);

  const handleSetTickRate = useCallback((value) => {
    if (useBackend) backend.setTickRateMs(value).catch(err => console.error('setTickRate failed', err));
    else setTickRate(value);
  }, [useBackend, backend]);

  const handleReset = useCallback(() => {
    if (useBackend) backend.reset().catch(err => console.error('reset failed', err));
    else localResetSimulation();
  }, [useBackend, backend]);

  const handleInjectEvent = useCallback((type) => {
    if (useBackend) backend.injectEvent(type).catch(err => console.error('injectEvent failed', err));
    else localInjectEvent(type);
  }, [useBackend, backend]);

  const handleRunBenchmark = useCallback((numMessages = 300000) => {
    setBenchmarking(true);
    setBenchResult(null);
    if (useBackend) {
      backend.runBenchmark(numMessages)
        .then(result => { setBenchResult(result); setBenchmarking(false); })
        .catch(err => { console.error('benchmark failed', err); setBenchmarking(false); });
    } else {
      localRunBenchmark(numMessages);
    }
  }, [useBackend, backend]);
    ...
    <div className="sp-root">
      ...
      <div className="sp-header">
        ...
        <div className="sp-live-dot">
          <i />
          {effectiveRunning
            ? `LIVE · ${useBackend ? 'BACKEND' : 'LOCAL'} · TICK ${effectiveTick}`
            : `PAUSED (${useBackend ? 'BACKEND' : 'LOCAL'})`}
        </div>
      </div>
      ...
      <div className="sp-content">
        {activePage === 'dashboard' && <DashboardPage ships={effectiveShips} alerts={effectiveAlerts} statsHistory={effectiveStatsHistory} liveStats={effectiveLiveStats} onNav={setActivePage} selectedShipId={selectedShipId} setSelectedShipId={setSelectedShipId} />}
        {activePage === 'map' && <LiveMapPage zones={effectiveZones} ships={effectiveShips} selectedShipId={selectedShipId} setSelectedShipId={setSelectedShipId} trailsOn={trailsOn} tick={effectiveTick} />}
        {activePage === 'fleet' && <FleetPage ships={effectiveShips} selectedShipId={selectedShipId} setSelectedShipId={setSelectedShipId} zones={effectiveZones} />}
        {activePage === 'alerts' && <AlertsPage alerts={effectiveAlerts} />}
        {activePage === 'ecosystem' && <EcosystemPage ships={effectiveShips} zones={effectiveZones} />}
        {activePage === 'ai' && <AIBehaviourPage ships={effectiveShips} />}
        {activePage === 'analytics' && <AnalyticsPage ships={effectiveShips} alerts={effectiveAlerts} statsHistory={effectiveStatsHistory} />}
        {activePage === 'geofence' && <GeofencePage zones={effectiveZones} setZones={handleSetZones} ships={effectiveShips} tick={effectiveTick} />}
        {activePage === 'simulator' && <SimulatorPage running={effectiveRunning} setRunning={handleSetRunning} shipTarget={effectiveShipTarget} setShipTarget={handleSetShipTarget} onReset={handleReset} onInject={handleInjectEvent} liveStats={effectiveLiveStats} ships={effectiveShips} onBenchmark={handleRunBenchmark} benchmarking={benchmarking} benchResult={benchResult} />}
        {activePage === 'settings' && <SettingsPage trailsOn={trailsOn} setTrailsOn={setTrailsOn} soundOn={soundOn} setSoundOn={setSoundOn} tickRate={effectiveTickRate} setTickRate={handleSetTickRate} backendConnected={useBackend} />}
      </div>
    </div>
  );
}
